...
class SystemSettings extends DBConnection{
	...
	function update_settings_info(){
		...
		if (isset($_FILES['cover']) && $_FILES['cover']['tmp_name'] != '') {
			...
			if ($move) {
				$versionedFileName = $fname . '?v=' . time();
		
				if (isset($_SESSION['system_info']['cover'])) {
					$qry = $this->conn->query("UPDATE system_info SET meta_value = '{$versionedFileName}' WHERE meta_field = 'cover'");
					if (is_file('../' . $_SESSION['system_info']['cover'])) {
						unlink('../' . $_SESSION['system_info']['cover']);
					}
				} else {
					$qry = $this->conn->query("INSERT INTO system_info (meta_value, meta_field) VALUES ('{$versionedFileName}', 'cover')");
				}
			}
		}
